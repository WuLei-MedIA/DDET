...
def bottleneck_block_pvp(x, filters, strides=(1, 1, 1), downsample=False):
    shortcut = x
...
def resnet50_pvp(input_shape):
    ...
    # Stage 2
    x = bottleneck_block_pvp(x, 64, downsample=True)
    for _ in range(2):
        x = bottleneck_block_pvp(x, 64)
    stage2_conv = x
    stage2 = layers.GlobalAveragePooling3D()(x)

    # Stage 3
    x = bottleneck_block_pvp(x, 128, strides=(2, 2, 2), downsample=True)
    for _ in range(3):
        x = bottleneck_block_pvp(x, 128)
    stage3_conv = x
    stage3 = layers.GlobalAveragePooling3D()(x)

    # Stage 4
    x = bottleneck_block_pvp(x, 256, strides=(2, 2, 2), downsample=True)
    for _ in range(5):
        x = bottleneck_block_pvp(x, 256)
    stage4_conv = x
    stage4 = layers.GlobalAveragePooling3D()(x)

    # Stage 5
    x = bottleneck_block_pvp(x, 512, strides=(2, 2, 2), downsample=True)
    for _ in range(2):
        x = bottleneck_block_pvp(x, 512)
    stage5_conv = x
